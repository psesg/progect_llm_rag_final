...
def split_image_text_types(docs):
    ...
    for doc in docs:
        ...
        if looks_like_base64(doc) and is_image_data(doc):
            ...
            texts.append(doc)
    print(f"split_image_text_types:\nlen(texts) = {len(texts)}\nlen(b64_images) = {len(b64_images)}")
    print(texts)
...
def multi_modal_rag_chain(retriever):
    ...
    print(f"multi_modal_rag_chain dict.get('context')=<<<{dict.get('context')}>>>")
    chain = (
        dict
...
def multi_modal_worag_chain(retriever):
    ...
    print(f"multi_modal_worag_chain dict.get('context')=<<<{dict.get('context')}>>>")
    chain = (
        dict
        | RunnableLambda(img_prompt_func)
        | gen_ai_model
...
# по текстовым данным
# "Что можно сказать о технологическом лидерстве Сбера на основании его годового отчета за 2023 год?"
